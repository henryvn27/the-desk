...
const data=await mkdtemp(join(tmpdir(),"desk-retrieval-"));
const output=resolve("artifacts/retrieval");
...
async function askTyped(text){await lens.mouse.move(150,150);await lens.mouse.down();await lens.mouse.move(360,150);await lens.mouse.move(360,300);await lens.mouse.move(150,300);await lens.mouse.up();await lens.getByLabel("Ask Lens",{exact:true}).fill(text);await lens.getByLabel("Ask Lens",{exact:true}).press("Enter");}
try{
 await launch();
 ...
 await page.evaluate(async()=>{
   ...
   await window.desk.command({type:"source.create",input:{title:"Vector reference",text:"Introductory material. ".repeat(400)+"Static friction prevents relative sliding. Maximum friction depends on the normal force."+"Other material. ".repeat(400),classIds:[course.id],taskIds:[task.id]}});
 ...
 await askTyped("How does static friction depend on normal force?");
 ...
 assert.equal(context.sources[0].title,"Vector reference");
 const passage=context.sources[0];
 assert.ok(passage.excerptStart>0);
 assert.ok(passage.excerpt.includes("Static friction prevents relative sliding."));
 assert.equal(passage.selectionMethod,"lexical-match");
 const original=(await page.evaluate(()=>window.desk.snapshot())).sources.find(s=>s.title==="Vector reference").text;
 assert.equal(passage.excerpt,original.slice(passage.excerptStart,passage.excerptEnd));
 ...
 console.log("PASS: later matching source passage included with exact original offsets; unrelated source excluded; persisted Guide/Direct modes control main-process request; provider action payload rejected; no extra task created; Lens mode survives restart. Responses are synthetic, not teaching-quality proof.");
