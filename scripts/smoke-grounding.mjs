...
const data=await mkdtemp(join(tmpdir(),"desk-grounding-"));
const output=resolve("artifacts/grounding");
...
try{
 ...
 console.log("PASS: task-linked source included and unrelated source excluded; persisted Guide/Direct modes control main-process request; provider action payload rejected; no extra task created; Lens mode survives restart. Responses are synthetic, not teaching-quality proof.");
