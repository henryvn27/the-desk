...
const data=await mkdtemp(join(tmpdir(),"desk-memory-evidence-")),output=resolve("artifacts/memory-evidence");await mkdir(output,{recursive:true});
...
try{
 ...
 const confirmed=(await page.evaluate(()=>window.desk.snapshot())).memories.find(m=>m.origin==="inferred");
 assert.equal(confirmed.evidence.observations[0].estimatedMinutes,30);
 await page.evaluate(async()=>{const session=(await window.desk.snapshot()).sessions[0];await window.desk.command({type:"session.review",id:session.id,notes:"Corrected after confirmation",remainingMinutes:null});});
 await page.getByText("Evidence · 3 reviewed tasks",{exact:true}).click();
 await page.getByText("Study history changed or original evidence is unavailable. This pattern is paused in tutoring; review or forget it.",{exact:true}).waitFor();
 assert.deepEqual((await page.evaluate(()=>window.desk.snapshot())).memories.find(m=>m.origin==="inferred").evidence,confirmed.evidence);
 ...
 console.log("PASS: original evidence retained after review correction and stale warning visible; duration pattern reviewed/confirmed, learning toggle, explicit edit, clear inferred preserves explicit note and sessions, clear survives restart. Timing is synthetic; no provider calls.");
