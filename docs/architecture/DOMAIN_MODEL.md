...
| Attempt | `attempts` JSON records; class FK in payload; optional same-class task and concept IDs | Explicit checked result, aid/hint use, notes and timestamp; contributes bounded concept counters |
...
The Session Kit is a read-only projection of the current assignment and explicitly associated local sources. It separates sources linked to the task from class-wide sources with no task association; sources belonging only to another task are excluded even within the same class. The latest three nonblank ended-session review notes are shown for this task, using insertion order. Same-class mistakes and linked or weak/due concepts are shown as explicit review context, along with the five most recent explicitly recorded attempts for the assignment. The kit is available in a pre-start preview and in the main active-session surface; compact controller contents remain compact. HTTPS resource buttons use the existing trusted main-process task-resource boundary. The kit does not infer verified mistakes, mastery, checklist completion or readiness from notes or attempts.
...
Schema 24 fences inferred-memory controls. Settings persist learning enabled and excluded session IDs. Duration candidates require three reviewed, unchanged, completed single-session tasks of the same class/work kind. Candidates are computed, not stored. memory.confirm recomputes the evidence basis in the trusted store, rejects stale/disabled/duplicate candidates and persists origin=inferred, source session IDs, sample count and median ratio. Edits retain inference provenance. Forgetting an inferred note excludes its evidence; clear-inferred removes inferred notes and excludes existing sessions while preserving explicit notes/tasks/session history. Any excluded session excludes its whole task history, preventing partial history from appearing to be a single-session task. These are duration patterns only; broader preference inference remains open.
...
Schema 27 adds durable `concepts`. Each record survives individual assignments, belongs to an existing class, may link many same-class tasks, and stores one of the contract's concept statuses, preparedness states, course/long-term retention mode, review due, attempts, unaided-correct/total, hint count, last-reviewed time and an evidence note. Create/edit/forget compare revisions transactionally; duplicate names are rejected within a class and impossible unaided totals are rejected at the boundary. Linked, weak or due concepts influence deterministic planning and appear in Session Kit as read-only context. This slice records explicit evidence; it does not infer mastery, completion, cumulative recall or final-exam readiness automatically.

Schema 28 adds durable `attempts`. Each record belongs to an existing class, may link one same-class task and many same-class concepts, and stores a checked result, unaided/aided state, bounded hint count, notes and attempted-at timestamp. Create/edit/forget compare revisions transactionally; cross-class links and aided attempts without a hint are rejected. Linked concept counters are updated atomically and reversibly, while completion, submission, mastery and preparedness remain separate fields. The Attempts view exposes the record for inspection and correction, and Session Kit shows recent assignment-linked attempts as read-only evidence. This slice does not infer categorical mastery, automatic preparedness, cumulative recall or final-exam retention from attempts.
