...
- Chat is the default desktop decision surface over those same canonical projections. `packages/intelligence/chat.ts` resolves bounded local intents and routes only unresolved explanations through the trusted main-process OpenRouter/Luna boundary; it does not create a chat-only academic store or a second action system. See `docs/architecture/CHAT_FIRST_POST_V1.md`.
