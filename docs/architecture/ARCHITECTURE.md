...
- `packages/planner` is deterministic. It handles confirmed deadlines, saved local study windows, a seven-day horizon, minimum useful blocks and honest unallocated work. The post-V1 layer derives calibrated duration ranges, cautious time-window evidence, assessment spacing, stable repair decisions and non-persisted what-if evaluations without replacing the scheduler. See `docs/architecture/PLANNER_POST_V1.md`.
- Home is a thin projection over the same Planner state. `packages/planner/home.ts` owns section ordering, attention signal filtering and exact continuation targets; it does not calculate a second priority score. See `docs/architecture/HOME_POST_V1.md`.
- Chat is the default desktop decision surface over those same canonical projections. `packages/intelligence/chat.ts` resolves bounded local intents and routes only unresolved explanations through the trusted main-process `AIProviderRouter`; it does not create a chat-only academic store or a second action system. See `docs/architecture/CHAT_FIRST_POST_V1.md` and `docs/architecture/AI_PROVIDERS.md`.
- Class is a thin projection over the existing academic graph. `packages/domain/class-experience.ts` composes canonical Class, Unit, Task, Assessment, Concept, Source, Teacher and grade records with Planner and Student Model evidence; it does not introduce a Course model or duplicate academic truth. See `docs/architecture/CLASS_POST_V1.md`.
- Local SQLite remains authoritative. The outbox stores an immutable payload for each local operation (including schema-43 unassigned Notes, Source annotations/revisions, Student Model and StudyActivity data), and the trusted Electron main process can append account-scoped copies to the approved Supabase operation-log table after authentication. Remote data is never applied silently; newer remote copies become preserved local conflicts that require an explicit decision. The production migration and remote-copy application remain separate release work.
- Library reading is a presentation over the V1 Source table and global search path. The post-V1 reader stores passage annotations and Note backlinks on the Source row, keeps exact source-relative anchors, and marks old revisions stale without creating a second source database. See `docs/architecture/LIBRARY_POST_V1.md`.
- Capture quality is an extension of the V1 Capture Inbox, parser, policy, provenance and outbox transaction. The post-V1 corpus measures routing, dates, calibration, duplicate review and durable recovery before adding deterministic object hints, browser quick capture and contextual class evidence. See `docs/architecture/CAPTURE_POST_V1.md`.
...
- Browser context is a separate untrusted-data boundary. `packages/integrations/browser-bridge.ts` accepts only versioned Chrome/Edge page-context envelopes with bounded HTTP(S) URL, title, selection and visible text fields. The trusted main process exposes a loopback-only, per-run bearer-token host at `/v1/browser/context`; the packaged MV3 extension sends only after explicit popup review and approved `chrome-extension://` origin validation. The envelope can be formatted as Lens evidence, but it cannot carry commands, credentials or page-execution requests. Real Chrome/Edge installation, page permission behavior and live-site adapters remain separate verification gates.
